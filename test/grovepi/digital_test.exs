defmodule GrovePi.DigitalTest do
  ...
  test "set_pin_mode to input", %{prefix: prefix, board: board} do
    GrovePi.Digital.set_pin_mode(prefix, @pin, :input)

    assert <<5, @pin, @input, 0>> == GrovePi.I2C.get_last_write_data(board)
  end

  test "set_pin_mode to output", %{prefix: prefix, board: board} do
    GrovePi.Digital.set_pin_mode(prefix, @pin, :output)

    assert <<5, @pin, @output, 0>> == GrovePi.I2C.get_last_write_data(board)
  end

  test "set_pin_mode to unsupported mode", %{prefix: prefix} do
    assert_raise FunctionClauseError, fn ->
      GrovePi.Digital.set_pin_mode(prefix, @pin, :unsupported)
    end
  end

  test "read requests a response and then reads a bit", %{prefix: prefix, board: board} do
  ...
  test "write writes a single bit", %{prefix: prefix, board: board} do
